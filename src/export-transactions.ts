...
async function main() {
  ...
  for (let i = 0; i < wallets.length; i++) {
    ...
    for (let t = 0; t < tokenAddresses.length; t++) {
      unresolved_transactions.push(getTransactions(name, tokenAddresses[t], network, address))
    }
  }
...
async function tagging(txs) {
  ...
  const MATIC_ORDER_TOPIC2 = '0x6869791f0a34781b29882982cc39e882768cf2c96995c2a110c577c53bc932d5'
  ...
  const marketOrdersTxs = await getTopicTxs(1, ethStartblock, ETH_ORDER_TOPIC)
  ...
  const maticOrdersTxs = await getTopicTxs(137, maticStartblock, MATIC_ORDER_TOPIC)
