import { Network, Networks } from '../entities/Networks'
...
import {
  TokenPriceAPIData,
  TokenPriceData,
} from '../interfaces/Transactions/TokenPrices'
import {
  TransferItem,
  TransferType,
} from '../interfaces/Transactions/Transfers'
import {
  baseCovalentUrl,
  COVALENT_API_KEY,
  fetchCovalentURL,
...
import {
  ...
  Status,
} from '../interfaces/GovernanceProposal'
...
export async function getTokenPrices(latestBlocks?: LatestBlocks) {
  ...
  for (const priceItem of priceData) {
    prices[priceItem.contract_address.toLowerCase()] = priceItem.prices.reduce(
      (accumulator, info) => {
...
export async function getTransactions(
  ...
  const url = `${baseCovalentUrl(
    network
  )}/address/${address}/transfers_v2/?key=${COVALENT_API_KEY}&contract-address=${tokenAddress}${startBlock >= 0 ? `&starting-block=${startBlock + 1}` : ''
    }`
  console.log('url', url)
    ...
    const transfers = tx.transfers.map((txTransfer) => {
      const type =
        WALLET_ADDRESSES.has(txTransfer.from_address) &&
          ...
          : txTransfer.transfer_type

      const date = tx.block_signed_at.split('T')[0]
      const contractAddress = txTransfer.contract_address.toLowerCase()
      const usdPrice = fullFetch
        ? priceData[contractAddress][date]
        : BALANCES[contractAddress]?.rate
      const amount = parseNumber(
        Number(txTransfer.delta),
        BALANCES[contractAddress]?.decimals || txTransfer.contract_decimals
      )

      const usdValue = usdPrice
        ? usdPrice * amount
        : txTransfer.delta_quote || 0
...
async function getTopicTxs(network: Network, startblock: number, topic: Topic) {
  ...
  const response = await fetchCovalentURL<{ height: number }>(
    `${baseCovalentUrl(network)}/block_v2/latest/?key=${COVALENT_API_KEY}`,
    PAGE_SIZE
  )
  ...
  console.log(
    `Latest ${JSON.stringify(
      network
    )} - start block: ${block} - latest block: ${latestBlock}`
  )
  ...
  while (block < latestBlock) {
    ...
    const url = `${baseCovalentUrl(
      network
    )}/events/topics/${topic}/?key=${COVALENT_API_KEY}&starting-block=${block}&ending-block=${endBlock > latestBlock ? 'latest' : endBlock
      }`
  ...
  try {
    ...
    console.log('error fetching', network, block, startblock, topic)
  }
}
...
PROJECTS.forEach((project) => {
  ...
  const isCommunityGrant =
    isGrant && !isCoreUnitCategory(project.configuration.category)
  const isOperationalGrant =
    isGrant && isCoreUnitCategory(project.configuration.category)

  if (isEnacted) {
    if (project.vesting_addresses.length > 0) {
      const vestingAddresses = project.vesting_addresses.map((address) =>
        address.toLowerCase()
      )
      if (isGrant) {
        vestingAddresses.forEach((address) =>
          GRANTS_VESTING_ADDRESSES.add(address)
        )
      }
      if (isBid) {
        vestingAddresses.forEach((address) =>
          BIDS_VESTING_ADDRESSES.add(address)
        )
      }

      if (isCommunityGrant) {
        vestingAddresses.forEach((address) =>
          COMMUNITY_GRANTS_VESTING_ADDRESSES.add(address)
        )
      }
      if (isOperationalGrant) {
        vestingAddresses.forEach((address) =>
          OPERATIONAL_GRANTS_VESTING_ADDRESSES.add(address)
        )
      }
    }
...
async function findSecondarySalesTag(txs: TransactionParsed[], chunk: number) {
  for (const tx of txs) {
    ...
    do {
      const url = `${baseCovalentUrl(
        Networks.get(tx.network)
      )}/transaction_v2/${tx.hash}/?key=${COVALENT_API_KEY}`
      try {
        const data = await fetchCovalentURL<TransactionItem>(url, PAGE_SIZE)
        fetched = true

        const log = data[0].log_events.find((log) =>
          Tags.isItemContract(log.sender_address)
        )
        ...
        console.log(`Chunk: ${chunk} - retrying...`, url, error)
        maxRetries--
      }
...
export async function tagging(txs: TransactionParsed[]) {
  ...
  const marketOrdersTxs = new Set(
    ethStartblock
      ? await getTopicTxs(ethNetwork, ethStartblock, Topic.ETH_ORDER_TOPIC)
      : []
  )
  ...
  const maticOrdersTxs = new Set(
    maticStartblock
      ? await getTopicTxs(
        polygonNetwork,
        maticStartblock,
        Topic.MATIC_ORDER_TOPIC
      )
      : []
  )
  ...
  const tagger = async (transactions: TransactionParsed[], chunk: number) => {
    for (let i = 0; i < transactions.length; i++) {
      ...
      if (
        tx.type === TransferType.OUT &&
        (COMMUNITY_GRANTS_VESTING_ADDRESSES.has(tx.to) ||
          COMMUNITY_GRANTS_ENACTING_TXS.has(tx.hash))
      ) {
      ...
      if (
        tx.type === TransferType.OUT &&
        (OPERATIONAL_GRANTS_VESTING_ADDRESSES.has(tx.to) ||
          OPERATIONAL_GRANTS_ENACTING_TXS.has(tx.hash))
      ) {
      ...
      if (
        ...
        GRANTS_VESTING_ADDRESSES.has(tx.from)
      ) {
      ...
      if (
        tx.type === TransferType.OUT &&
        (BIDS_VESTING_ADDRESSES.has(tx.to) || BIDS_ENACTING_TXS.has(tx.hash))
      ) {
        tx.tag = TagType.BID
        continue
      }
  ...
  const secondarySales = taggedTxns.filter(
    (tx) => tx.tag === TagType.SECONDARY_SALE
  )
  ...
  taggedTxns = taggedTxns.map((tx) => ({
    ...tx,
    ...taggedsecondarySales.find((ss) => tx.hash === ss.hash),
  }))
...
async function saveTransactionFiles(
  txs: TransactionParsed[],
  tagged = false,
  year?: number
) {
...
export async function saveTransactions(txs: TransactionParsed[], tagged = false) {
