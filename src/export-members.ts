...
async function main() {
  ...
  let votes: Vote[] = await fetchGraphQL(url, 'votes', where, 'created', 'voter')
  ...
  const membersInfo: Promise<void>[] = []

  for (let i = 0; i < members.length; i++) {
    membersInfo.push(getMemberInfo(members[i]))
  }

  await Promise.all(membersInfo)
